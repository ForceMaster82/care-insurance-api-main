...
@CareInsuranceWebMvcTest(CaregivingChargeModificationController::class)
class CaregivingChargeModificationControllerTest(
    ...
    context("간병 접수의 간병비 산정 수정 개요를 조회하면") {
        val request = get("/api/v1/receptions/01GPJMK7ZPBBKTY3TP0NN5JWCJ/caregiving-charge-modification")
        ...
        should("간병 접수의 간병비 산정 수정 개요를 페이로드에 포함하여 응답합니다.") {
            expectResponse(
                content().json(
                    """
                        {
