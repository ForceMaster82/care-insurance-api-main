...
@CareInsuranceWebMvcTest(DailySettlementTransactionStatisticsController::class)
class DailySettlementTransactionStatisticsControllerTest(
    ...
    context("일자별 간병비 정산으로 인한 입출금 통계를 조회하면") {
        val request = get("/api/v1/daily-settlement-transaction-statistics")
        ...
        beforeEach {
            every {
                dailySettlementTransactionStatisticsByDateQueryHandler.getDailySettlementTransactionStatistics(
                    match {
        ...
        should("단일 날짜의 정산금 입출금 통계를 목록 형식으로 페이로드에 포함하여 응답합니다.") {
            expectResponse(
                content().json(
                    """
                        [
        ...
        should("정산금 입출금 통계 조회를 도메인 영역에 위임합니다.") {
