...
object CaregivingChargeModificationHistoryAccessPolicy : AccessPolicy {
    override fun check(sub: Subject, act: Action, obj: Object) {
        ...
        } else if (actionType.contains(ActionType.READ_ONE)) {
            denyIfNotInternalUser(sub)
        } else {
    ...
    private fun denyIfNotInternalUser(sub: Subject) {
        ...
        throw AccessDeniedException("간병비 산정 수정 기록 접근하기 위해서는 내부 사용자 권한이 필요합니다.")
    }
}
