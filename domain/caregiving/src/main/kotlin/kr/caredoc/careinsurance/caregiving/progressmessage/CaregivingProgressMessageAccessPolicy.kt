...
object CaregivingProgressMessageAccessPolicy : AccessPolicy {
    override fun check(sub: Subject, act: Action, obj: Object) {
        ...
        if (actionType.contains(ActionType.CREATE)) {
            ...
            throw AccessDeniedException("허용되지 않는 행위입니다.")
        }
    }
    ...
    private fun denyIfNotInternalUser(sub: Subject) {
        ...
        throw AccessDeniedException("간병 진행 메시지 접근하기 위해서는 내부 사용자 권한이 필요합니다.")
    }
}
