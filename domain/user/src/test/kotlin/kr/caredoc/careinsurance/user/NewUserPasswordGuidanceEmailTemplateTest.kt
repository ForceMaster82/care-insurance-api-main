...
class NewUserPasswordGuidanceEmailTemplateTest : BehaviorSpec({
    given("템플릿과 데이터가 주어졌을때") {
        val template = NewUserPasswordGuidanceEmailTemplate()
        val data = NewUserPasswordGuidanceEmailTemplate.TemplateData(
        ...
        `when`("이메일을 생성하면") {
            ...
            then("생성된 이메일은 고정된 제목을 가진다.") {
                ...
                generatedEmail.title shouldBe "[케어 인슈어런스] 가입 및 로그인 안내"
            }
        ...
        `when`("또 다른 데이터로 이메일을 생성하면") {
            val anotherData = NewUserPasswordGuidanceEmailTemplate.TemplateData(
                userEmail = "leo@caredoc.kr",
                userName = "레오",
                rawPassword = "4Q3w2e1r!!",
            )
